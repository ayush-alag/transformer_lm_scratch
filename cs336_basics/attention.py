...
from .transformer import softmax, Linear
...
class MultiheadSelfAttention(nn.Module):
   def __init__(self, d_model, num_heads, rope_theta=None, max_seq_len=None, token_positions=None, device=None):
      super().__init__()
      ...
      self.w_qkv = Linear(d_model, 3 * num_heads * self.d_kv, device=device) # store q, k, v together
      self.w_o = Linear(self.d_kv * num_heads, d_model, device=device)
